...
const loadHead = () => {
    ...
    fetch("/html/head.html")
        ...
        .then(data => {
            ...
            template.content.querySelectorAll("meta, link").forEach(node => {
                if (node.tagName === "META") {
                    ...
                    if (nameAttr && document.head.querySelector(`meta[name="${nameAttr}"]`)) {
                        node.remove();
                    }
                } else if (node.tagName === "LINK") {
                    const relAttr = node.getAttribute("rel");
                    const hrefAttr = node.getAttribute("href");
                    if (relAttr && hrefAttr && document.head.querySelector(`link[rel="${relAttr}"][href="${hrefAttr}"]`)) {
                        node.remove();
                    }
                }
            });
        ...
        .catch(error => console.error("Erreur lors du chargement du head :", error));
};
...
const highlightActiveLink = () => {
    ...
    if (!linkByPath.has(targetPath)) {
        ...
        if (
            targetPath.startsWith("/html/") &&
            !nonProductPages.has(targetPath)
        ) {
